...
func TestDailyCanReceiveDailyResponse(test *testing.T) {
	...
	`"latitude":51.4620184,"longitude":-0.1152248},"status":"new","ticket_status":"available",` +
	...
	mux.HandleFunc("/events/daily.json", func(w http.ResponseWriter, r *http.Request) {
...
func TestDailyProvidesCorrectQuery(test *testing.T) {
	httpClient, mux, server := testServer()
	defer server.Close()
	...
	if(path != "/events/daily.json"){test.Errorf("expected path to be /events/search.json, got %v", path)}
